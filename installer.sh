...
REPO_URL="http://f1144757.xsph.ru/YuMino_1.2.zip"
REPO_NAME="YuMino_1.2.zip"
EXTRACTED_DIR="YuMino"

if [ -d "$EXTRACTED_DIR" ]; then
  echo "$EXTRACTED_DIR уже существует."
  mv "$EXTRACTED_DIR" "${EXTRACTED_DIR}.bak.$(date +%Y%m%d%H%M%S)"
fi

if ! command -v unzip &> /dev/null
then
  echo "unzip не найден. Пожалуйста, установите unzip перед запуском скрипта. установка..."
  pkg install unzip
  echo"unzip был установлен. скрипт завершён"
  exit 1
fi

echo "Скачивание юзербота..."
wget "$REPO_URL" -O "$REPO_NAME"
if [ $? -ne 0 ]; then
  echo "Ошибка при скачивании юзербота."
  exit 1
fi
echo "юзербот скачан"
...
if [ $? -ne 0 ]; then
    echo "Ошибка при распаковке юзербота.  Убедитесь, что установлен unzip и архив не поврежден."
    rm "$REPO_NAME"
    exit 1
fi
